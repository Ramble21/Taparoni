...
def get_batch(split, size, return_wnns=False, return_index=False):
  ...
  if return_wnns:
    ...
    batch_wnns = torch.stack([features_raw_x[i] for i in index]).to(DEVICE)
  ...
  for i in range(1, num_steps + 1):
    ...
    if model_type == 'pretrain':
      ...
      if len(pretrain_targets_f) == 0:
        ...
        preds, loss = model(pieces_batch_f, colors_batch_f, ttm_batch_f, index=index_f, targets=pretrain_targets_f)
    else:
...
def load_saved_weights():
  ...
  m_eval.load_state_dict(torch.load("../data/saved_weights.pt"))
  m_eval.to(DEVICE)
  m_eval.eval()
  return m_eval
